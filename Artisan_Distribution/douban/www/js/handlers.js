...
function aboutPage() {
    ...
    slide_in(html, rABOUT, '.frame', function() {
        ...
        $('#add_feedback').click(function() {
            tr('showFeedbackPanel');
            if (user) {
                var user_id = window.localStorage.getItem('user_id');
                plugins.Feedback.showFeedbackPanel(user_id);
            } else {
                plugins.Feedback.showFeedbackPanel(null);
            }
        });
    });
}
...
function appsPage() {
    if (isIDevice) {
        ...
        $.getp(API + '/promote_apps?platform=android', true, function(o) {
                var html = $.tmpl('#tmpl_apps', {apps: o});
                slide_in(html, rAPPS, '.frame', function() {
                    new iScroll('wrapper_apps');
                });
    ...
    /*var html = $.tmpl('#tmpl_apps');
    slide_in(html, rAPPS, '.frame', function() {
        new iScroll('wrapper_apps');
        plugins.SoftKeyBoard.init();
    });*/
}
...
function handleDownloadUrlRedirect(url) {
    tr("handleDownloadUrlRedirect url:" + url);
    plugins.Redirect.redirect(url);
}
